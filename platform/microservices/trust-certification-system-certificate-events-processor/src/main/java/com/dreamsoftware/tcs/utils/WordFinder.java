...
abstract class WordFinder implements OnWordFoundCallback {
    ...
    /**
     * Checks if XWPFDocument tables contain a given bookmark. Checks runs of
     * all paragraphs if searchable text is in one or scattered in runs around
     * it. It does not check separate paragraphs if text is scattered amongst
     * them.
     *
    ...
    /**
     * Checks if XWPFDocument text contains a given bookmark. Checks runs of all
     * paragraphs if searchable text is in one or scattered in runs around it.
     * It does not check separate paragraphs if text is scattered amongst them.
     *
     * @param doc XWPFDocument
     * @param word to be searched
     */
    void findWordsInText(XWPFDocument doc, String word) {
        this.bookmark = word;
        doc.getParagraphs().stream().filter((p) -> (paragraphNotNullAndHasRuns(p))).forEachOrdered((p) -> {
            checkInParagraph(p);
        });
    }
    ...
    private void checkCell(XWPFTableCell cell) {
        ...
        cell.getParagraphs().stream().filter((p) -> (paragraphNotNullAndHasRuns(p))).forEachOrdered((p) -> {
            checkInParagraph(p);
        });
    }
