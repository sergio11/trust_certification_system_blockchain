...
import com.dreamsoftware.tcs.web.validation.constraints.ShouldHaveEnoughFundForIssuingCertificate;
 ...
 * @author ssanchez
 */
public class ShouldHaveEnoughFundForIssuingCertificateValidator implements ConstraintValidator<ShouldHaveEnoughFundForIssuingCertificate, String> {
    ...
    public boolean isValid(String courseEditionId, ConstraintValidatorContext context) {
        ...
        if(principal != null) {
            certificationCourseEditionRepository.findById(new ObjectId(courseEditionId)).ifPresent((certificationCourseEditionEntity -> {
                try {
                    isValid.set(tokenManagementBlockchainRepository.getMyTokens(principal.getWalletHash()) >= certificationCourseEditionEntity.getCostOfIssuingCertificate());
