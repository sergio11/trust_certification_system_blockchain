...
import com.dreamsoftware.tcs.stream.events.course.EnableCertificationCourseEditionEvent;
import com.dreamsoftware.tcs.stream.events.notifications.course.CourseEditionEnabledNotificationEvent;
...
@Slf4j
public class EnableCertificationCourseEditionHandler extends AbstractProcessAndReturnHandler<EnableCertificationCourseEditionEvent, CourseEditionEnabledNotificationEvent> {
     ...
     */
    @Override
    public CourseEditionEnabledNotificationEvent onHandle(final EnableCertificationCourseEditionEvent event) throws RepositoryException {
        ...
        log.debug("EnableCertificationCourseEditionHandler CALLED!");
        if(certificationCourseEditionEntity.getStatus() != CertificationCourseStateEnum.DISABLED) {
            throw new IllegalStateException("Course Edition can not be enabled");
        }
        certificationCourseBlockchainRepository.disable(event.getCaWalletHash(), certificationCourseEditionEntity.getId().toString());
        certificationCourseEditionRepository.updateStatus(certificationCourseEditionEntity.getId(), CertificationCourseStateEnum.ENABLED);
        return CourseEditionEnabledNotificationEvent.builder()
