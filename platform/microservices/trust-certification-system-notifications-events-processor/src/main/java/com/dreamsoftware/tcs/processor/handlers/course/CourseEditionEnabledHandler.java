...
@Slf4j
public class CourseEditionEnabledHandler extends AbstractNotificationHandler<CourseEditionEnabledNotificationEvent> {
     ...
     */
    @Override
    public void onHandle(final CourseEditionEnabledNotificationEvent notification) {
        Assert.notNull(notification, "Notification can not be null");
        log.debug("CourseEditionEnabledHandler CALLED!");
    ...
    private void sendNotification(final UserEntity caMemberEntity, final CertificationCourseEditionEntity courseEditionEntity) {
        ...
        final String notificationTitle = resolveString("course_edition_enabled_title", userLocale, new Object[]{ courseTitle });
        final String notificationMessage = resolveString("course_edition_enabled_message", userLocale, new Object[]{ courseTitle });
        notificationService.saveNotification(notificationTitle, notificationMessage, caMemberEntity);
        mailClientService.sendMail(CourseEditionEnabledMailRequestDTO
                .builder()
